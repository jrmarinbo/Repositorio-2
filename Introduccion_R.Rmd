...
#GENERO UN VECTOR X CON VALORES ALEATORIOS DE UNA FUNCION DE DISTRIBUCION NORMAL CON MEDIA = 0 Y DESVIACION ESTANDAR = 1
# --------------------------- #
x
...
R/ rnorm ES UNA FUNCION DE DISTRIBUCION NORMAL
...
```{R inputExample}
source("D:/GitHub/Repositorio-2/example2.R")
```
```{R outputExample, echo=FALSE}
source("D:/GitHub/Repositorio-2/example3.R")
```
...
weight <-c(4.4,5.3,7.2,5.2,8.5,7.3,6,10.4,10.2,8.1)
...
R/
abs() genera el valor absoluto de un numero
sqr() genera la raiz cuadrada de un numero
...
for (number in array)
  {
    ...
    print(paste(number,"no es par",sep = " "))
